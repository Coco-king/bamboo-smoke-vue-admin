    ...
    >
      ...
      <el-form-item label="用户此次登录的Access Token" prop="accessToken">
        <el-input
          ...
          placeholder="用户此次登录的Access Token"
        ></el-input>
      </el-form-item>
      <el-form-item
        label="针对QQ，在授权自动续期步骤中，获取新的Access_Token时需要提供的参数"
        prop="refreshToken"
      >
        <el-input
          v-model="dataForm.refreshToken"
          placeholder="针对QQ，在授权自动续期步骤中，获取新的Access_Token时需要提供的参数"
        ></el-input>
      </el-form-item>
      <el-form-item label="Access Token过期时间" prop="expiresIn">
        <el-input
          v-model="dataForm.expiresIn"
          placeholder="Access Token过期时间"
        ></el-input>
      </el-form-item>
    ...
    return {
      ...
      dataRule: {
        ...
        accessToken: [
          {
            required: true,
            message: '用户此次登录的Access Token不能为空',
            trigger: 'blur'
          }
        ],
        refreshToken: [
          {
            required: true,
            message:
              '针对QQ，在授权自动续期步骤中，获取新的Access_Token时需要提供的参数不能为空',
            trigger: 'blur'
          }
        ],
        expiresIn: [
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(
              `/admin/membersocial/info/${this.dataForm.id}`
            ),
            method: 'get',
    ...
    dataFormSubmit() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/admin/membersocial/${!this.dataForm.id ? 'save' : 'update'}`
            ),
