<template>
  <el-dialog
    ...
    <el-form-item label="父级分类的ID" prop="parentId">
      <el-input v-model="dataForm.parentId" placeholder="父级分类的ID"></el-input>
    </el-form-item>
    <el-form-item label="SEO关键字" prop="metaKeywords">
      <el-input v-model="dataForm.metaKeywords" placeholder="SEO关键字"></el-input>
    </el-form-item>
    <el-form-item label="SEO描述内容" prop="metaDescription">
      <el-input v-model="dataForm.metaDescription" placeholder="SEO描述内容"></el-input>
    </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/api/category/info/${this.dataForm.id}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/api/category/${!this.dataForm.id ? 'save' : 'update'}`),
